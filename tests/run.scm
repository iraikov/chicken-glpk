...
(let ((lpp (make-problem 'maximize pbounds xbounds objcoefs constraints)))
  (scale-problem lpp)
  (use_presolver lpp #t)
  (let ((status (simplex lpp)))
    (assert (= status 200))
    (assert (< (abs (- (get-objective-value lpp) 733.33333333333333)) 1e-12))
    (assert (every (lambda (x y) (< (abs (- x y)) 1e-12))
		   (f64vector->list (get-column-primals lpp))
